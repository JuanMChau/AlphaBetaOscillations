...
fs = filesep;
datapath  = [path '/data_neural/processed'];
datapath_behav = [path '/data_behavioural/experiment_matrix_format'];
datapath_triggers = [path '/data_neural/triggers'];

addpath([path '/toolbox']);
addpath(genpath([path '/toolbox/plt']));
addpath([path '/toolbox/stat']);
...
for roi = 1:size(averageERPs,4)
    % Select data to plot
    plotdata = averageERPs(:,itime,:,roi);

    %smooth data?
    fsample = round(median(1./diff(timelist)));
    ftype    = 'gaussian';
    fsize    = 0.012*fsample;
    plotdata = filtfast(plotdata,2,[],ftype,fsize);

    % open figure
    clc
    figure
    % color settings
    plotcol       = colormap(linspecer(size(plotdata,3),'qualitative'));
    SetupPlotting();
    ...
    for isig = 1:nsig
        ...
        [datobs, datrnd] = cluster_test_helper(testdata(:,it,isig)', nperm,'t');
    ...
    cfg = [];
    hplot = plotpatch(plotdata,plottime,plotcol,cfg);
    ...
    fname = sprintf('%s/%s%s',figpath,'reward_erp_',roiNames{roi});
